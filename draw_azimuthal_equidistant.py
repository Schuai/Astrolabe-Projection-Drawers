...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Draw a horizon azimuthal-equidistant SVG for a given observer latitude."
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("azimuthal_equidistant_projection.svg"),
...
def build_svg(args: argparse.Namespace) -> ElementTree:
    ...
    pole_sign = 1.0 if args.center == "north" else -1.0
    outer_angle = 90.0 - pole_sign * args.range_latitude
    radius_scale = canvas_radius / outer_angle
